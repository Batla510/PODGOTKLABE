...
li_reg.innerHTML = `<a href='/registration' class='menu__item' onclick='regis(event)'>Reg</a>`;
...
function showCart(event){
    ...
    carts.forEach(cart => {
        ...
        `<div class='cart__card'>
            ...
            <p class='cart__price'>${cart.product.price} Рублей</p>
         </div>
        `)
...
async function regis(event){
    ...
    `<div class='container'>
        <form class='regform'>
            ...
            <button class='btn__reg' onclick='registrationFFF(event)'>Отправить</button>
        </form>
    </div>
    `)
    ...
    login.addEventListener('focusout',()=> {
       if (login.value.length < 8){
        document.querySelector('.login__error').innerHTML='СЛИШКАМ МАЛО БУКФ'
       }
    })
    let email = formReg[1]
    ...
    email.addEventListener('focusout',()=> {
       if (!email.value.includes('@')){
            document.querySelector('.email__error').innerHTML='НЕПРАВИЛЬНЫЙ ЕМАИЛ,ПЕРЕДЕЛЫВАЙ'
       }
    })
    let pass = formReg[2]
    ...
    pass.addEventListener('focusout',()=> {
       if (pass.value.length < 8){
            document.querySelector('.pass__error').innerHTML='ПАРОЛЬ ДОЛЖЕН БЫТЬ БОЛЬШЕ ИЛИ РОВНО 8 СИМВОЛОВ'
       }
    })
    let pass2 = formReg[3]
    ...
    pass2.addEventListener('focusout',()=> {
       if (pass2.value != pass.value){
            document.querySelector('.pass2__error').innerHTML='ПАРОЛИ НЕ СОВПАДАЮТ'
       }
    })
...
async function logFunc(event){
    ...
    `<div class='container'>
        <form class='login_form'>
            ...
            <button class='btn__login' onclick='logInInIn(event)'>Отправить</button>
        </form>
    </div>
    `)
